...
mata:
	...
	simViews = asarray_create("real",2)
...
mata:
	d=1
	    ...
	    uvtoAdd=J(rows(statIdLong),6,.)
        uvtoAddg=J(rows(statIdLong),6,.)
        uvtoAddb=J(rows(statIdLong),6,.)
        vuvtoAdd=J(rows(statIdLong),6,.)
        uvre1toAdd=J(rows(statIdLong),6,.)
        uvre2toAdd=J(rows(statIdLong),6,.)	
        gameList=J(0,2,.)
end
...
mata:
		i=58
			...
			marketp=marketIdLong[i]
			
			/* Data needed for the viewership model */
